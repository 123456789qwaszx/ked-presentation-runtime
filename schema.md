...
## portrait-dimensions.json

`PortraitGeneratedDB`의 실제 스프라이트 rect를 `(character, variant, emotion)`별로
내보낸 치수 표다. 같은 캐릭터도 variant나 emotion에 따라 종횡비가 다를 수 있으므로
캐릭터 하나의 대표 종횡비로 합치지 않는다.

| 필드 | 의미 |
|---|---|
| `character` | 정규화 전 캐릭터 키. 조회 시 trim + 소문자 정규화 |
| `variant` | DB의 variant 키(예: `yoonsaea_b`). resolver와 같이 마지막 글자를 접미사로 사용 |
| `emotion` | 표정 코드. resolver와 같이 `1` → `01`, `2` → `02`로 정규화 |
| `width` / `height` | `sprite.rect`의 픽셀 치수 |

`HeightFitPreserveAspect`의 확정 크기는 현재 fold 트리에서 초상 이미지 부모의 높이를
구한 뒤 `sizeDelta.x = parentHeight × (width / height)`, `sizeDelta.y = 0`으로 계산한다.
조회 순서는 런타임 `PortraitResolver`와 같다: 정확한 character/variant/emotion을 먼저
찾고, 없으면 같은 캐릭터의 기본 `a/01`로 폴백한다. 둘 다 없으면 `Unhandled`이다.
