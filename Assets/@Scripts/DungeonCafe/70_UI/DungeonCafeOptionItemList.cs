...
public readonly struct DungeonCafeOptionEntry
{
    public readonly string Label;
    public readonly bool IsAvailable;

    public DungeonCafeOptionEntry(string label, bool isAvailable = true)
    {
...
/// <summary>
/// VNOptionItem 인스턴스 묶음을 관리.
///
/// 게스트하우스 화면은 전부 '라벨 목록에서 하나 고르기' 형태라 생성/정리/포커스 로직이 같음.
/// 다섯 곳에 같은 코드를 두지 않도록 여기로 모으고, 각 화면은 라벨 조립만 담당.
///
/// VNOptionItem은 Selectable 이라 마우스 hover 와 패드 입력이 그대로 동작.
/// 대신 Submitted 가 항목 자신을 돌려주므로, 인덱스 환원은 이쪽 책임.
///
/// MonoBehaviour가 아님.
/// 패널이 필드로 들고 수명을 직접 관리.
/// </summary>
public sealed class DungeonCafeOptionItemList
{
    // 선택된 항목의 인덱스
    ...
    /// 한 번 고르면 목록을 잠글지 여부.
    /// 결정을 확정하는 목록은 true,
    /// 업무수첩처럼 열람을 반복하는 목록은 false
    /// </summary>
    public bool LockOnSubmit { get; set; } = true;

    // 패널 OnInitialize 에서 한 번 호출한다. 템플릿은 여기서 꺼 둔다.
    ...
    public void Rebuild(IReadOnlyList<DungeonCafeOptionEntry> entries)
    {
        Clear();
        ...
        for (int i = 0; i < entries.Count; i++)
        {
            ...
            // Awake에서 ResetView 를 돌리기에, 활성화한 뒤에 바인딩해야 값이 남음.
    ...
    // 통제 상실처럼 목록은 남기되 입력만 막는 용도.
    // 표시를 지우면 무엇을 잃었는지 보이지 않으므로 항목 자체도 유지.
    ...
    /// </summary>
    private static VNOptionViewModel BuildViewModel(in DungeonCafeOptionEntry entry, int index)
    {
    ...
    // 패드/키보드로도 바로 진행할 수 있도록 첫 후보에 포커스.
