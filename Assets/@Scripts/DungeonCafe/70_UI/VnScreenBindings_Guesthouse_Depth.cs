...
public sealed partial class VnScreenBindings
{
    ...
    public async YarnTask<IReadOnlyList<string>> RequestDepthInterventionAsync(DepthInterventionRequestV3 request)
    {
        ...
        RefreshGuesthouseHud(request.Session, "붕괴심층");
        ...
        ServiceSessionStateV3 session, DepthRollResult roll, IReadOnlyList<string> postRollAbilityIds)
    {
        ...
        if (string.IsNullOrEmpty(_pendingRollAbilityId) || _guesthouseCampaign == null)
            return DepthRollDecisionV3.None;

        PlayerAbilityDefinition def = _guesthouseCampaign.Content.GetAbility(_pendingRollAbilityId);
        if (def == null)
            return DepthRollDecisionV3.None;

        return def.EffectKind == AbilityEffectKind.DepthBandDowngrade
            ? new DepthRollDecisionV3(reroll: null, downgrade: _pendingRollAbilityId)
            : new DepthRollDecisionV3(reroll: _pendingRollAbilityId, downgrade: null);
    }
