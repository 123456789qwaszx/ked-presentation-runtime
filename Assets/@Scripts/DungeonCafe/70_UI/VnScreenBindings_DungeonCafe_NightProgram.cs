...
// (밤 처리 계획)
// manageCount 명까지 (메이드, 안정/관리 붕괴) 를 담아 확정.
// 빈 목록 = 전원 방치.
public sealed partial class VnScreenBindings
{
    private bool _isWaitingNightPlan;
    private bool _hasNightPlanResult;
    private IReadOnlyList<NightChoice> _pendingNightPlan;

    public async YarnTask<IReadOnlyList<NightChoice>> RequestNightPlanAsync(NightPlanRequest request)
    {
        if (_isWaitingNightPlan)
            throw new InvalidOperationException("밤 처리 선택을 이미 기다리고 있습니다.");

        RefreshDungeonCafeHud("밤 - 처리 선택");
    ...
    private void HandleNightPlanConfirmed(IReadOnlyList<NightChoice> plan)
    {
