...
// 콘텐츠 정의 레코드 모음. 전부 불변
// 저작은 DungeonCafeContent(코드 내장)가 기본이고, SO 이관 시 각 레코드 1:1 매핑한다.

/// <summary>메이드 정의. </summary>
...
public sealed class ServiceOption
{
    ...
    public ServiceOption(OptionIntensity intensity, BurdenAxis loadAxis, string nodeKey, string tag = null)
    {
        Intensity = intensity;
        LoadAxis = loadAxis;
...
/// <summary>심층 결과표: 4구간 + 개체 고유 특수 1. </summary>
...
public sealed class MonsterProfile
{
    ...
    public IReadOnlyList<ServiceBeat> Beats { get; }
    ...
    public MonsterProfile(
        ...
        IReadOnlyList<ServiceBeat> beats, DepthActionSet depthActions,
...
/// <summary>종족 규약: 파국 노드. 탈출 경로는 전 종족 공통 = 회수 구간뿐. </summary>
public sealed class SpeciesProtocol
{
    ...
    public SpeciesProtocol(MonsterSpecies species, string displayName, string controlLossNode, string endingNode)
    {
...
/// <summary>후유증 정의. tremor·brand. 붕괴 0(tremor) 또는 관리붕괴(brand)로 해소. </summary>
...
/// <summary>기벽 정의. 계약 4필드: 효과 / 판정 보정 / 대사 세트 / 밤 이벤트. </summary>
...
/// <summary>능력 해금 4-튜플 + 사용 제한. </summary>
...
/// <summary> 콘텐츠 루트. 조회는 전부 여기로.</summary>
public sealed class DungeonCafeContentDB
{
    private readonly Dictionary<string, MaidProfile> _maids = new(StringComparer.Ordinal);
    private readonly Dictionary<string, MonsterProfile> _monsters = new(StringComparer.Ordinal);
    private readonly Dictionary<MonsterSpecies, SpeciesProtocol> _protocols = new();
    ...
    public IReadOnlyList<MaidProfile> Maids { get; }
    public IReadOnlyList<MonsterProfile> Monsters { get; }
    ...
    {
        Maids = maids; 
        foreach (MaidProfile m in maids)
            _maids[m.MaidId] = m;
        
        Monsters = monsters; 
        foreach (MonsterProfile m in monsters)
            _monsters[m.MonsterId] = m;
        
        foreach (SpeciesProtocol p in protocols)
    ...
    public List<MonsterProfile> GetMonsterPool(int dayNumber)
    {
