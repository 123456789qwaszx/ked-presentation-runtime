...
public static class DungeonCafeRuleSelfCheck
{
    public static string RunAll(out bool allPassed)
    {
        ...
        DungeonCafeTuning tuning = DungeonCafeTuning.CreateStandard();
        ...
        {
            int at86 = SettlementRule.Calculate(
                SettlementOutcomeKind.Normal, 7, 70, 60, 86, tuning).Energy;
            int at74 = SettlementRule.Calculate(
        ...
        {
            SettlementResult at99 = SettlementRule.Calculate(
                SettlementOutcomeKind.Normal, 7, 70, 60, 99, tuning);
            SettlementResult escaped = SettlementRule.Calculate(
        ...
        {
            SettlementResult r = SettlementRule.Calculate(
        ...
        allPassed = failed == 0;
        report.Insert(0, $"DungeonCafe 자가 검증 - {(allPassed ? "전체 통과" : $"{failed}건 실패")}\n");
