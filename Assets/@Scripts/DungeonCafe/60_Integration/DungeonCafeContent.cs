...
// 코드 내장 콘텐츠. guesthouse_design_v3.md §1, §9~§13 표를 그대로 데이터화.
// SO 저작 파이프라인 도입 시 이 파일이 폴백으로 남는다.
public static class DungeonCafeContent
{
    public static DungeonCafeContentDB Build()
    {
        return new DungeonCafeContentDB(
            BuildMaids(), BuildMonsters(), BuildProtocols(),
            BuildAftereffects(), BuildQuirks(), BuildAbilities(), BuildCalendar());
    }

    // ---- 메이드  ----
    private static List<MaidProfile> BuildMaids() => new()
    {
        new("maid_shion", "시온", 1, new AxisTriple(4, 2, 1), "guarded",
            traitRecoveryShift: 8, 
            traitRiskyShift: 0,
            statusModifierPercent: 100,
            dispositionChancePercent: 30,
            dispositionKey: "training"),
        
        new("maid_arie", "아리에", 2, new AxisTriple(1, 4, 2), "clinical",
            traitRecoveryShift: 0, 
            traitRiskyShift: 0,
            statusModifierPercent: 50,
            dispositionChancePercent: 25, 
            dispositionKey: "archiving"),
        
        new("maid_rui", "루이", 4, new AxisTriple(2, 1, 4), "dependent",
            traitRecoveryShift: 0,
            traitRiskyShift: -3,
            statusModifierPercent: 100,
            dispositionChancePercent: 35,
            dispositionKey: "greeting"),
    };

    // ---- 몬스터 ----
    private static List<ServiceBeat> Beats(string monsterId, BurdenAxis axis, string heavyTag)
    {
        var beats = new List<ServiceBeat>(3);
        for (int i = 1; i <= 3; i++)
        {
    ...
    private static List<MonsterProfile> BuildMonsters() => new()
    {
    ...
    // ---- 종족 설명  ----
    private static List<SpeciesProtocol> BuildProtocols() => new()
    {
