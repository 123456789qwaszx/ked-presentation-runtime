...
public sealed class DungeonCafeSaveModel
{
    ...
    public CampaignState Restore(DungeonCafeContentDB content, DungeonCafeTuning tuning)
    {
        var campaign = new CampaignState(content, tuning, 0UL);
        ...
        if (Enum.TryParse(phase, out CampaignPhase parsed)) campaign.Phase = parsed;

        for (int i = 0; i < maids.Count; i++)
        {
            MaidSave ms = maids[i];
            MaidState m = campaign.GetMaid(ms.maidId);
            if (m == null) continue;
