...
public sealed class ServiceSessionFlow
{
    ...
    public ServiceSessionFlow(CampaignState campaign, VnScreenBindings screens, IDungeonCafeNodePlayer dungeonCafeNodes)
    {
        _campaign = campaign; _screens = screens; _dungeonCafeNodes = dungeonCafeNodes;
    }

    public async YarnTask<SettlementResult> RunAsync(MaidState maid, MonsterProfile monster)
    {
        _campaign.Phase = CampaignPhase.InService;
        _campaign.Abilities.StartNewService();

        var session = new ServiceSessionState(maid, monster, Content.GetProtocol(monster.Species));
        ...
        while (session.DepthBeatCount < maxBeats)
        {
            ...
            if (playerControlled)
            {
                ...
                IReadOnlyList<string> chosen = await _screens.RequestDepthInterventionAsync(
                    new DepthInterventionRequest(session, session.DepthBeatCount, layout,
    ...
    private DepthRollResult RollDepth(
        ServiceSessionState session, in DepthBandLayout layout,
    ...
    private async YarnTask<DepthRollResult> OfferPostRollAsync(
        ServiceSessionState session, DepthRollResult roll, DepthBandLayout layout)
    {
        while (true)
        {
            ...
            DepthRollDecision decision = await _screens.PresentDepthRollAsync(session, roll, post);
    ...
    private DepthBand PredictBand(
        in DepthBandLayout layout, MaidState maid, MonsterProfile monster, UnderstandingTier tier)
    {
