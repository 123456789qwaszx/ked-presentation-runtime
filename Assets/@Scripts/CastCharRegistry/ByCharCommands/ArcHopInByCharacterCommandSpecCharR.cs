...
[Serializable]
[CommandMenuHint("Char Rig Motion", "ArcHop In By Character", Order = -759)]
public sealed class ArcHopInByCharacterCommandSpecCharR : CommandSpecBase
{
...
public sealed class ArcHopInByCharacterCommandCharR : CommandBase, IStepScopedCommand
{
    private readonly ArcHopInByCharacterCommandSpecCharR _spec;
    ...
    public ArcHopInByCharacterCommandCharR(ArcHopInByCharacterCommandSpecCharR spec) => _spec = spec;
    ...
    private void ResolveRefs(CommandRunScope scope)
    {
        ...
        {
            if (_spec.strict)
                Debug.LogError("[ArcHopInByCharacterCommandCharR] characterKey is null or empty.");
            return;
        }
            ...
            string.IsNullOrWhiteSpace(roleKey))
        {
            if (_spec.strict)
                Debug.LogWarning(
                    $"[ArcHopInByCharacterCommandCharR] No cast role found for character='{characterKey}'.");
            return;
        }
        ...
        if (!scope.Refs.TryGetCharRigRefs(roleKey, out CharacterRigRefs rig) || rig == null)
        {
            if (_spec.strict)
                Debug.LogWarning(
                    $"[ArcHopInByCharacterCommandCharR] Rig refs not found. character='{characterKey}', roleKey='{roleKey}'.");
            return;
        }
        ...
        if (_rect == null)
        {
            if (_spec.strict)
                Debug.LogWarning(
                    $"[ArcHopInByCharacterCommandCharR] Target rect not found. character='{characterKey}', roleKey='{roleKey}', target='{_spec.target}'.");
            return;
        }
