...
public sealed class FadeInByCharacterCommandSpecCharR : CommandSpecBase
{
...
public sealed class FadeInByCharacterCommandCharR : CommandBase, IStepScopedCommand
{
    private readonly FadeInByCharacterCommandSpecCharR _spec;
    ...
    public FadeInByCharacterCommandCharR(FadeInByCharacterCommandSpecCharR spec) => _spec = spec;
    ...
    private void ResolveRefs(CommandRunScope scope)
    {
        ...
        {
            if (_spec.strict)
                Debug.LogError("[FadeInByCharacterCommandCharR] characterKey is null or empty.");
            return;
        }
            ...
            string.IsNullOrWhiteSpace(roleKey))
        {
            if (_spec.strict)
                Debug.LogWarning(
                    $"[FadeInByCharacterCommandCharR] No cast role found for character='{characterKey}'.");
            return;
        }
        ...
        if (!scope.Refs.TryGetCharRigRefs(roleKey, out CharacterRigRefs rig) || rig == null)
        {
            if (_spec.strict)
                Debug.LogWarning(
                    $"[FadeInByCharacterCommandCharR] Rig refs not found. character='{characterKey}', roleKey='{roleKey}'.");
            return;
        }
