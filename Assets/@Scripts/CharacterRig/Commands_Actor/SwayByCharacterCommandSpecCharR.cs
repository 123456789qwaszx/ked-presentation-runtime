...
[Serializable]
[CommandMenuHint("Char Rig Motion", "Sway By Character", Order = 101)]
public sealed class SwayByCharacterCommandSpecCharR : CommandSpecBase
{
...
public sealed class SwayByCharacterCommandCharR : CommandBase, IStepScopedCommand
{
    private readonly SwayByCharacterCommandSpecCharR _spec;
    ...
    public SwayByCharacterCommandCharR(SwayByCharacterCommandSpecCharR spec) => _spec = spec;
    ...
    private void ResolveRefs(CommandRunScope scope)
    {
        ...
        {
            if (_spec.strict)
                Debug.LogError("[SwayByCharacterCommandCharR] characterKey is null or empty.");
            return;
        }
            ...
            string.IsNullOrWhiteSpace(roleKey))
        {
            if (_spec.strict)
                Debug.LogWarning(
                    $"[SwayByCharacterCommandCharR] No cast role found for character='{characterKey}'.");
            return;
        }
        ...
        if (!scope.Refs.TryGetCharRigRefs(roleKey, out CharacterRigRefs rig) || rig == null)
        {
            if (_spec.strict)
                Debug.LogWarning(
                    $"[SwayByCharacterCommandCharR] Rig refs not found. character='{characterKey}', roleKey='{roleKey}'.");
            return;
        }
        ...
        if (_rect == null)
        {
            if (_spec.strict)
                Debug.LogWarning(
                    $"[SwayByCharacterCommandCharR] Target rect not found. character='{characterKey}', roleKey='{roleKey}', target='{_spec.target}'.");
            return;
        }
