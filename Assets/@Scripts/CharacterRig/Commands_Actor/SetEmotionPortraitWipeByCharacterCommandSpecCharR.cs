...
[Serializable]
[CommandMenuHint("Char Rig", "Set Emotion Portrait (Overlay Wipe) By Character", Order = -960)]
public sealed class SetEmotionPortraitWipeByCharacterCommandSpec : CommandSpecBase
{
...
public sealed class SetEmotionPortraitWipeByCharacterCommand : CommandBase, IStepScopedCommand
{
    private readonly SetEmotionPortraitWipeByCharacterCommandSpec _spec;
    ...
    public SetEmotionPortraitWipeByCharacterCommand(
        SetEmotionPortraitWipeByCharacterCommandSpec spec,
    ...
    protected override IEnumerator ExecuteInner(CommandRunScope scope)
    {
        ...
        {
            Debug.LogWarning(
                $"[SetEmotionPortraitWipeByCharacterCommand] Failed to resolve portrait:\n" +
    ...
    private void ResolveRefs(CommandRunScope scope)
    {
        ...
        {
            if (_spec.strict)
                Debug.LogError("[SetEmotionPortraitWipeByCharacterCommand] characterKey is null or empty.");
            return;
        }
            ...
            string.IsNullOrWhiteSpace(roleKey))
        {
            if (_spec.strict)
                Debug.LogWarning(
                    $"[SetEmotionPortraitWipeByCharacterCommand] No cast role found for character='{characterKey}'.");
            return;
        }
        ...
        if (!scope.Refs.TryGetCharRigRefs(roleKey, out CharacterRigRefs rig) || rig == null)
        {
            if (_spec.strict)
                Debug.LogWarning(
                    $"[SetEmotionPortraitWipeByCharacterCommand] Rig refs not found. character='{characterKey}', roleKey='{roleKey}'.");
            return;
        }
    ...
    private static CanvasGroup GetRootCanvasGroup(RectTransform root, string debugName)
    {
        ...
        throw new InvalidOperationException(
            $"[SetEmotionPortraitWipeByCharacterCommand] CanvasGroup missing on Root: {debugName} ({root.name})");
    }
