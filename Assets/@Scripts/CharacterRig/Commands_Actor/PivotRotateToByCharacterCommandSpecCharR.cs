...
[Serializable]
[CommandMenuHint("Char Rig Motion", "PivotRotate To By Character", Order = 102)]
public sealed class PivotRotateToByCharacterCommandSpecCharR : CommandSpecBase
{
...
public sealed class PivotRotateToByCharacterCommandCharR : CommandBase, IStepScopedCommand
{
    private readonly PivotRotateToByCharacterCommandSpecCharR _spec;
    ...
    public PivotRotateToByCharacterCommandCharR(PivotRotateToByCharacterCommandSpecCharR spec) => _spec = spec;
    ...
    private void ResolveRefs(CommandRunScope scope)
    {
        ...
        {
            if (_spec.strict)
                Debug.LogError("[PivotRotateToByCharacterCommandCharR] characterKey is null or empty.");
            return;
        }
            ...
            string.IsNullOrWhiteSpace(roleKey))
        {
            if (_spec.strict)
                Debug.LogWarning(
                    $"[PivotRotateToByCharacterCommandCharR] No cast role found for character='{characterKey}'.");
            return;
        }
        ...
        if (!scope.Refs.TryGetCharRigRefs(roleKey, out CharacterRigRefs rig) || rig == null)
        {
            if (_spec.strict)
                Debug.LogWarning(
                    $"[PivotRotateToByCharacterCommandCharR] Rig refs not found. character='{characterKey}', roleKey='{roleKey}'.");
            return;
        }
        ...
        {
            Debug.LogWarning(
                $"[PivotRotateToByCharacterCommandCharR] Target rect not found. character='{characterKey}', roleKey='{roleKey}', target='{_spec.target}'.");
        }
    }
