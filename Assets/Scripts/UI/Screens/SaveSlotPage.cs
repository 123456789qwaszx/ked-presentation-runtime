...
public abstract class SaveSlotPage : UIPage<SaveSlotPage.Refs>
{
    ...
    private void CacheRefs()
    {
        ...
        _firstPageButton = View.Button(Refs.FirstPageButton_Button);
        _previousPageButton = View.Button(Refs.PreviousPageButton_Button);
        _pageLabel = View.Text(Refs.PageLabel_Text);
        _nextPageButton = View.Button(Refs.NextPageButton_Button);
        _lastPageButton = View.Button(Refs.LastPageButton_Button);
    ...
    public void Rebuild(VNSaveSlotMeta[] metas)
    {
        if (!_valid)
            return;
    ...
    private void RefreshPage()
    {
        ...
        int startMetaIndex = _pageIndex * slotsPerPage;

        for (int i = 0; i < _slotButtons.Count; i++)
        {
            VNSaveSlotButton slotButton = _slotButtons[i];
    ...
    private void RefreshPagingButtons()
    {
        ...
        bool canGoPrevious = _pageIndex > 0;
        bool canGoNext = _pageIndex < lastPageIndex;

        _firstPageButton.interactable = canGoPrevious;
        _previousPageButton.interactable = canGoPrevious;
        _nextPageButton.interactable = canGoNext;
        _lastPageButton.interactable = canGoNext;
    }
    ...
    private void CreateSlotButtonsIfNeeded()
    {
        ...
        while (_slotButtons.Count < slotsPerPage)
        {
            ...
            slotButton.Clicked += HandleSlotClicked;
            _slotButtons.Add(slotButton);
        }

        while (_slotButtons.Count > slotsPerPage)
        {
            int lastIndex = _slotButtons.Count - 1;
            VNSaveSlotButton slotButton = _slotButtons[lastIndex];
