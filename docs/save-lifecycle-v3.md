...
후속 5·6·7 구현으로 수동 슬롯 덮어쓰기/삭제, 미참조 회차 정리, catalog, 단건 본문 복구, 주기적 재시도를 추가했다.
현재 정책과 서버 계약은 [save-manual-slots.md](save-manual-slots.md)를 기준으로 한다.
기존 형식으로의 역호환이나 기존 로컬 저장 이전은 지원하지 않는다. 병렬 업로드는 추가하지 않았다.
...
- `bookmarks.json`: 수동 슬롯 요약, 서버 삭제 대기, 로컬 삭제 tombstone.
- `bookmark-snapshots/{key}.json`: 수동 슬롯 본문.
- `playthrough-catalog.json`: 회차 목록 파생 캐시.
...
앱 시작/커밋과 Unity Update 유지보수에서 재시도한다. 회차별 5~300초 backoff를 디스크에 보존한다.
영구 4xx는 중단 후 명시적 재시도를 요구한다. 자세한 기준은 수동 저장 문서를 참조한다.
...
서버의 명시적인 이어하기 포인터에서 snapshot 한 개만 복구하고, 북마크는 요약 페이지를 받는다.
active가 이미 있거나 사용자가 새 게임/fork를 선택했다면 늦은 응답으로 active를 변경하지 않는다.
기존 로컬 파일도 덮지 않는다. 선택한 북마크 본문만 단건 복구한다.

이어하기 복구와 북마크 요약 복구의 완료 상태를 따로 저장하고 cursor로 부분 복구를 재개한다.
이어하기 startup 장벽에는 resume 요청만 포함하고 북마크 목록은 포함하지 않는다.
요약 목록 복구가 진행 중이면 미발견 참조 보호를 위해 회차 정리를 미룬다.
...
최초 구현은 아래 17개 시나리오로 검증했다. 후속 수동 슬롯/복구 정책 변경 후 현재 harness는 25개 시나리오다.
로컬 검증은 .NET 8 Roslyn 직접 컴파일 및 .NET 런타임 실행으로 통과했다.
...
기존 Unity 테스트는 사용자로부터 통과 보고를 받았다. 이번 추가 변경은 이 환경에서 Unity Editor 및 실제 Spring 서버로 실행하지 않았다.
새로운 서버 계약을 반영한 일괄 체크리스트는 수동 저장 문서에 있으며, 아래 기존 실물 확인 항목도 참고할 수 있다.
...
콘텐츠 버전 호환성/마이그레이션, 손상 파일 복구와 저장 오류 UI는
